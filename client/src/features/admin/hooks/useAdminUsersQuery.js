...
const fetchAdminUsers = async ({ q = "", page = 1, limit = DEFAULT_LIMIT } = {}) => {
  const query = buildSearchParams({ q, page, limit });
  const response = await http.get(`/admin/users${query}`);
  return normalizeUsersPayload(response);
};

export const useAdminUsersQuery = (
  { q = "", page = 1, limit = DEFAULT_LIMIT } = {},
  options = {}
) =>
  useQuery({
    queryKey: adminKeys.users({ q, page, limit }),
    queryFn: () => fetchAdminUsers({ q, page, limit }),
